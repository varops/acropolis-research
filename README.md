...
**Dataset licences (checked 2026-09-17).** LongMemEval is released under the MIT licence (repository and Hugging Face dataset card). BEAM's code is MIT and its benchmark data is CC BY-SA 4.0. LoCoMo is CC BY-NC 4.0, non-commercial. The paper's v1.1 plan was to publish the unredacted reports only if all three allow redistribution of excerpts with attribution; LoCoMo's non-commercial term means they do not all allow it, so the redaction below stays in place for every benchmark and the hashes remain the verification path. An unredacted LongMemEval-only or BEAM-only bundle (the latter under CC BY-SA for those files) would be licence-compatible and is a separate decision, not taken here.
